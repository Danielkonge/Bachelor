...
\begin{align}\label{eq:Deltas}
  ...
  \end{aligned}                 
\end{align}
for $\xi\in R_\ell$. Here as in the relations for the $D$'s earlier, we replace $D_+D_-$ with $E_+D_+D_-E_-$ for $\xi \in R_{\ell,\ell}$ and $E_-D_+D_-E_+$ for $\xi\in R_{\ell,-\ell}$. Alternatively by \cref{eq:Drels}, cf.\ \Cref{sec:Deltacalc}, we also get that

...
for $w,w'\in V(2)$ and $v,v'\in V(n)$. Now by straightforward calculations, cf.\ \Cref{sec:innerproductscalc}, we get that
\begin{align}\label{eq:inprodres}
  \inner{s_0}{s_0} = 1, \qquad \inner{t_0}{t_0} = \dfrac{n(n+2)}{2}, \qquad \inner{u_0}{u_0} = \dfrac{n^2(n+1)(n-1)}{4}.
\end{align}

Now set $\overline w_k = w_k/\norm{w_k}$, $\overline v_k = v_k/\norm{v_k}$, $\overline s_k = s_k/\norm{s_k}$, $\overline t_k = t_k/\norm{t_k}$, and $\overline s_k = s_k/\norm{s_k}$ for all possible $k$, where $\norm{}$ is given by $\norm{v}=\sqrt{\inner{v}{v}}$ as usually in an inner product space. Note that \todo{Show the following equations --- I can show these by long calculations, but I think there is an easier way}
\begin{align*}

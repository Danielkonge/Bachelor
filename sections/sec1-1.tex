...
Now set $\overline w_k = w_k/\norm{w_k}$, $\overline v_k = v_k/\norm{v_k}$, $\overline s_k = s_k/\norm{s_k}$, $\overline t_k = t_k/\norm{t_k}$, and $\overline s_k = s_k/\norm{s_k}$ for all possible $k$, where $\norm{}$ is given by $\norm{v}=\sqrt{\inner{v}{v}}$ for $v\in V(2)\otimes V(n)$ as usually in an inner product space. Note that \todo{Show the following equations --- I can show these by long calculations, but I think there is an easier way}
\begin{align*}
...
\end{align*}
Likewise we get that $w_2=[h_-,\sqrt{2}f_3]=\sqrt{2}f_-$, so we can take our basis to be $(w_0,w_1,w_2)=(-\tfrac{\sqrt{2}}{2}f_+,\sqrt{2}f_3,\sqrt{2}f_-)$ when thinking of $L_p$ as the $L_k$-module $M(2)$. Normalizing as in \cref{eq:normedbases1}, we get that $(\overline w_0,\overline w_1,\overline w_2)=(-\tfrac{\sqrt{2}}{2}f_+,f_3,\sqrt{2}f_-)$. So by \cref{eq:Drels}, we see that in $L_p\otimes M(2\ell)$
\begin{align*}
...
\end{align*}
So if $\ell_0=0$ choosing a constant $c$ such that
\begin{align*}
  \dd{\ell_0}{0} = \dfrac{c}{\ell_0(\ell_0+1)},
\end{align*}
we see inductively that if

...
\end{align*}
We want to describe the $s_k$'s more explicitly. First note that if $n>0$\todo{Maybe I will move some calculations of $s_k$, $t_k$, and $u_k$ to an appendix}
\begin{align*}
...
\end{align*}
We likewise see that for $k=n+1$ the last term vanishes, so we have $s_{k+1}=w_2\otimes v_{n-1}+ w_1\otimes v_n$, and for $k=n+2$ the two last terms vanish, so we get $s_{k+2}=w_2\otimes v_n$. Thus altogether we have that
\begin{align}
  \begin{aligned}
    s_0 &= w_0\otimes v_0, \\
...
% We see that
% \begin{align*}
%   u_1 &= y.\bigl( w_0\otimes v_2 - \dfrac{n-1}{2}w_1\otimes v_1 + \dfrac{n(n-1)}{2}w_2\otimes v_0 \bigr) \\
%       &= y.w_0\otimes v_2 + w_0\otimes y.v_2 - \dfrac{n-1}{2}y.w_1\otimes v_1 - \dfrac{n-1}{2}w_1\otimes y.v_1 \\
%       &\phantom{{}={}}{} + \dfrac{n(n-1)}{2}w_2\otimes y.v_0 \\
%       &= w_1\otimes v_2 + 3w_0\otimes v_3 - (n-1)w_2\otimes v_1 - (n-1)w_1\otimes v_2 \\
%       &\phantom{{}={}}{} + \dfrac{n(n-1)}{2}w_2\otimes v_1 \\
%       &= 3w_0\otimes v_3 - \bigl((n-1)-1\bigr)w_1\otimes v_2 + \dfrac{n(n-1)-2(n-1)}{2}w_2\otimes v_1 \\
%   &= 3w_0\otimes v_3 - (n-2)w_1\otimes v_2 + \dfrac{(n-1)(n-2)}{2}w_2\otimes v_1,
% \end{align*}
...
Now we want to express $w_1\otimes v_k$ for $0\leq k\leq n$ in terms of the bases $(s_k \mid 0\leq k\leq n+2)$, $(t_k \mid 0\leq k\leq n)$, and $(u_k \mid 0\leq k\leq n-2)$. We see that for $0<k<n$\todo{This should probably be in an appendix}
\begin{align*}
